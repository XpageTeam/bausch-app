...
class LensBloc extends Bloc<LensEvent, LensState> {
  LensBloc() : super(LensInitial()) {
    on<LensEvent>((event, emit) async {
      if (event is LensUpdate) {
        emit(LensUpdated(model: event.model));
      }

      if (event is LensGet) {
        emit(LensGetting(model: state.model));
        emit(await getParameters());
      }

      if (event is LensSend) {
        emit(LensLoading(model: state.model));
        emit(await sendParameters(event.model));
      }
    });
    add(LensGet());
  }

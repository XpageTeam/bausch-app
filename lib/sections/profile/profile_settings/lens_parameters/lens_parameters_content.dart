...
class _LensParametersScreenState extends State<LensParametersContent> {
  final getLensCubit = GetLensCubit();
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<GetLensCubit, GetLensState>(
      bloc: getLensCubit,
      builder: (context, state) {
        if (state is GetLensSuccess) {
          return Scaffold(
        ...
        if (state is GetLensFailed) {
          return ErrorPage(
            title: state.title,
            subtitle: state.subtitle,
            buttonText: 'Обновить',
            buttonCallback: getLensCubit.getParameters,
          );
        }
